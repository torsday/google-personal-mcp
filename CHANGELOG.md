...
- Contacts **group membership write** `modify_contact_group_membership` ([ADR-0024 §Tool inventory](docs/adr/0024-contacts-service-surface.md)): the first Contacts *write* tool. Adds/removes members on a group via People API `contactGroups.members.modify` — `Aspect::Write`, requiring the `contacts` (read-write) scope (`contacts.readonly` is insufficient). At least one of `resource_names_to_add`/`resource_names_to_remove` must be non-empty; works on system groups (`myContacts`, `starred`) too. Does not create/delete groups (out of scope) or delete contacts. The API's per-member skips (`notFoundResourceNames`, `canNotRemoveLastContactGroupResourceNames`) are surfaced verbatim — the call succeeds even when some members are skipped (#211).
...
- Gmail **forwarded-attachment parse** `parse_forwarded_attachment` ([ADR-0026 §`parse_forwarded_attachment`](docs/adr/0026-gmail-tool-surface-phase-2.md)): parses a `message/rfc822` attachment (a forwarded email) into a nested message tree — `messages.get` (20 units) validates the part is `message/rfc822` and returns `Error::UnsupportedMimeType` otherwise, then `messages.attachments.get` fetches the raw bytes, which are parsed (via `mailparse`) into headers + body + attachment summaries, descending into any forwards nested *within* it. Recursion is bounded: a per-call `max_depth` (default 5) is clamped to the new `[services.gmail].parse_forwarded_max_depth_ceiling` (default 10), so a forward-within-forward chain cannot exhaust the stack. `Aspect::Read` on the existing `gmail.readonly` scope; every field at every nesting level is wrapped `_untrusted` per [ADR-0018](docs/adr/0018-email-content-trust.md) (#232).
...
### Added — Drive service surface
- Drive **service-module scaffold** ([ADR-0025](docs/adr/0025-drive-service-surface.md)): new `src/drive/` tree mirroring `gmail/`/`calendar/`/`contacts/` — `DriveClient<T>` (same `RefreshTransport` auth + single-retry 401 fallback as Gmail), a `DriveService` seam, and stub tool-group modules (`files`, `download`, `permissions`). `GoogleServer` gains an optional `drive` service, constructed at startup only when `[services.drive]` is enabled. Drive is the **first service with a per-account scope override**: `[services.drive.accounts.<alias>] scopes = [...]` overrides `[services.drive] scopes`, resolved by `ServiceEntry::resolve_account_scopes` and validated at load time against the allowed Drive set (`drive.file` / `drive.readonly` / `drive`) so a typo or over-broad grant fails the daemon at startup, not at first call. New `Error::ExportRequired` and `Error::UnsupportedExportType` variants for the Google-native-doc download/export paths. No Drive tools yet — they land in follow-ups (#213/#215/#217/#220) (#212).

### Changed
- `audit_summary`'s `recent_destructive` shortlist now tracks the formal `Aspect::Destructive` set (the single source of truth in `crate::tools::metadata`) instead of a duplicate hand-maintained "all mutating tools" list ([ADR-0022](docs/adr/0022-capability-gating.md); #235). **Contents change**: recoverable `write` tools (`archive_thread`, `batch_archive`, `modify_thread_labels`, `batch_modify_thread_labels`) no longer appear in `recent_destructive`, while `purge_account` (irreversible) now does. The operator's "what destructive things did I do" view is now exactly the irreversible/high-blast operations. No change to what the audit log *records* (#235).
