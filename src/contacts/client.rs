//! `PeopleClient` — the HTTP wrapper every Contacts-touching tool will call
//! into, per [ADR-0024](../../docs/adr/0024-contacts-service-surface.md).
//!
//! Mirrors [`crate::gmail::client::GmailClient`] and
//! [`crate::calendar::client::CalendarClient`]: generic over
//! [`crate::auth::tokens::RefreshTransport`], the same `new` shape, the same
//! `authed_get` / `authed_post` surface, and the single-retry 401 fallback per
//! [ADR-0004](../../docs/adr/0004-oauth-token-refresh.md).
//!
//! Targets the **People API** (`people.googleapis.com/v1`); the deprecated
//! Google Contacts v3 (gdata) API is not implemented (ADR-0024 §Technical Notes).
//!
//! Scaffold module: no Contacts tools call these methods yet — they land in
//! follow-up tickets (#206+). The module-level `allow(dead_code)` covers the
//! surface those tickets consume.
#![allow(dead_code)]
...
impl<T: RefreshTransport> PeopleClient<T> {
    /// `base_url` is the People API root with no trailing slash, e.g.
    /// [`PEOPLE_API_BASE`] in production or a wiremock host in tests.
    pub(crate) fn new(
    ...
    /// Share a rate limiter with other clients so a single per-account /
    /// per-project budget covers every Google call.
    ...
    /// Per-project then per-account quota check (project first so a denial
    /// never charges the per-account bucket). Mirrors the Gmail client.
        ...
        F: Fn(&str) -> reqwest::RequestBuilder + Send + Sync,
    {
        ...
        let resp = execute_with_retry("contacts", &self.retry, || async {
...
/// Strip the query string from a path for the `google.endpoint` tracing field.
fn endpoint_of(path: &str) -> &str {
    path.find('?').map_or(path, |i| &path[..i])
}

/// Bump `gmcp_google_api_calls_total` + duration histogram with
/// `service = "contacts"`. Mirrors the Gmail client's recorder.
...
async fn parse_json<R: DeserializeOwned>(resp: reqwest::Response) -> Result<R, Error> {
    ...
    if !status.is_success() {
        return Err(Error::upstream("contacts", status.as_u16(), text));
    }
    serde_json::from_str(&text).map_err(|e| Error::Parse {
        context: "contacts response body".to_owned(),
        source: e,
    })
}
...
mod tests {
    ...
    fn fresh_state(access: &str) -> TokenState {
        TokenState {
            ...
            scopes: vec!["https://www.googleapis.com/auth/contacts".into()],
            client_id: "cid".into(),
    ...
    fn make_client(
    ...
    ) -> Arc<PeopleClient<MockRefreshTransport>> {
        let responses = refresh_response
            .map(|body| vec![Ok((200, body))])
            .unwrap_or_default();
        let dir = std::env::temp_dir().join(format!("gpm-ppl-{}", std::process::id()));
        ...
        Arc::new(
            PeopleClient::new(base_url, tokens, reqwest::Client::new())
    ...
    async fn non_success_status_maps_to_upstream() {
        ...
        let client = make_client(&server.uri(), "ACCESS-1", None);
        let err = client
            .authed_get::<EchoBody>("work", "/people/me", 1)
            .await
            .expect_err("403 must error");
        assert!(
            matches!(err, Error::Upstream { ref service, status: 403, .. } if service == "contacts"),
            "got: {err:?}"
        );
    }
}
