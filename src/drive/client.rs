//! `DriveClient` — the HTTP wrapper every Drive-touching tool will call into,
//! per [ADR-0025](../../docs/adr/0025-drive-service-surface.md).
//!
//! Deliberately mirrors [`crate::calendar::client::CalendarClient`] (itself a
//! mirror of [`crate::gmail::client::GmailClient`]) so a contributor can grep
//! "how does Calendar do X" and apply it here: same generic over
//! [`crate::auth::tokens::RefreshTransport`], same `new` shape, same
//! `authed_get` / `authed_post` surface, and the same single-retry 401 fallback
//! (force-refresh once, replay once, then `Error::AuthRequired`) per
//! [ADR-0004](../../docs/adr/0004-oauth-token-refresh.md) §"401 fallback".
//!
//! Scaffold module: no Drive tools call these methods yet — they land in
//! follow-up tickets (#213 list/search, #215 download/export, #217 metadata,
//! #220 permissions). The module-level `allow(dead_code)` covers the surface
//! those tickets will consume; remove it as tools wire in.
#![allow(dead_code)]
...
/// Production Drive API root (no trailing slash). The v3 REST surface covers
/// `files.list`/`get`/`update` and `permissions`; the binary `files.get?alt=media`
/// download path and the `files.export` path also hang off this base.
pub(crate) const DRIVE_API_BASE: &str = "https://www.googleapis.com/drive/v3";

/// Per-call quota cost for a Drive API request. Drive meters in flat "queries"
/// against a per-100-seconds budget (no Gmail-style weighted units), so every
/// endpoint charges the shared rate limiter a single unit
/// ([ADR-0025](../../docs/adr/0025-drive-service-surface.md)).
pub(crate) const QUERY_COST: u32 = 1;

pub(crate) struct DriveClient<T: RefreshTransport = ReqwestRefreshTransport> {
    base_url: String,
    ...
    /// project-level check entirely. Shared with the Gmail/Calendar clients when
    /// all count against the same Google project.
    project_quota: Option<Arc<ProjectQuotaRegistry>>,
}

impl<T: RefreshTransport> DriveClient<T> {
    /// `base_url` is the Drive API root with no trailing slash, e.g.
    /// [`DRIVE_API_BASE`] in production or a wiremock host
    ...
    /// [`crate::calendar::client::CalendarClient`]'s `quota_check`.
        ...
        F: Fn(&str) -> reqwest::RequestBuilder + Send + Sync,
    {
        ...
        let resp = execute_with_retry("drive", &self.retry, || async {
...
/// with `service = "drive"`. Mirrors the Calendar client's recorder.
...
async fn parse_json<R: DeserializeOwned>(resp: reqwest::Response) -> Result<R, Error> {
    ...
    if !status.is_success() {
        return Err(Error::upstream("drive", status.as_u16(), text));
    }
    serde_json::from_str(&text).map_err(|e| Error::Parse {
        context: "drive response body".to_owned(),
        source: e,
    })
}
...
mod tests {
    ...
    fn make_client(
    ...
    ) -> Arc<DriveClient<MockRefreshTransport>> {
        let responses = refresh_response
            .map(|body| vec![Ok((200, body))])
            .unwrap_or_default();
        let dir = std::env::temp_dir().join(format!("gpm-drive-{}", std::process::id()));
        ...
        Arc::new(
            DriveClient::new(base_url, tokens, reqwest::Client::new())
    ...
    async fn success_path_passes_bearer_token() {
        ...
        let body: EchoBody = client
            .authed_get("work", "/files/abc123", 1)
            .await
            .expect("ok");
    ...
    async fn non_success_status_maps_to_upstream() {
        ...
        let client = make_client(&server.uri(), "ACCESS-1", None);
        let err = client
            .authed_get::<EchoBody>("work", "/files/abc123", 1)
            .await
            .expect_err("404 must error");
        assert!(
            matches!(err, Error::Upstream { ref service, status: 404, .. } if service == "drive"),
            "got: {err:?}"
        );
    }
}
